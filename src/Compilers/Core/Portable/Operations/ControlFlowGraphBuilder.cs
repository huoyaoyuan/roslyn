...
oneMoreTime:
        ...
        private void AddDisposingFinally(IOperation resource, bool requiresRuntimeConversion, ITypeSymbol iDisposable, IMethodSymbol? disposeMethod, ImmutableArray<IArgumentOperation> disposeArguments, bool isAsynchronous)
        {
            ...
            IOperation? tryDispose(IOperation value)
            {
                ...
                if (method != null)
                {
                    ...
                    if (isAsynchronous)
                    {
                        return new AwaitOperation(invocation, isConditional: false, semanticModel: null, value.Syntax, _compilation.GetSpecialType(SpecialType.System_Void), isImplicit: true);
                    }
        ...
        public override IOperation? VisitForEachLoop(IForEachLoopOperation operation, int? captureIdForResult)
        {
            ...
            IOperation getCondition(IOperation enumeratorRef)
            {
                if (info?.MoveNextMethod != null)
                {
                    ...
                    {
                        return new AwaitOperation(moveNext, isConditional: false, semanticModel: null, operation.Syntax, _compilation.GetSpecialType(SpecialType.System_Boolean), isImplicit: true);
                    }
                    return moveNext;
                }
                else
        ...
        {
            return new AwaitOperation(VisitRequired(operation.Operation), operation.IsConditional, semanticModel: null, operation.Syntax, operation.Type, IsImplicit(operation));
        }
